 ...
 * */
public class FoodCallWS extends FoodWebApiCall {

    //interface used to retrieve API response,
    // and to use response in requesting activities.
    private FoodDataFetchListener dataFetchListener;
     ...
     * @param dataFetchListener listener to receive result of an API
     * @see FoodDataFetchListener
     * */
    public FoodCallWS(Activity activity, String url, FoodDataFetchListener dataFetchListener){
     ...
     * @see FoodDataFetchListener
     * */
    public FoodCallWS(Activity activity, String url, String params, FoodDataFetchListener dataFetchListener){
        super(activity, url, true, WebUtil.RequestMethod.POST, params);
        this.dataFetchListener = dataFetchListener;
     ...
     * @see FoodDataFetchListener
     * */
    public FoodCallWS(Activity activity, String url, String reqMethod, String params, FoodDataFetchListener dataFetchListener){
...
//        System.out.print("Receiving data...Progress ===> " + progress);
    ...
    void onFinishSuccess(String response) {
        ...
        if (response.startsWith("failed to connect to") || response.startsWith("Unable to resolve")){
            FoodAppHelper.showToast(mActivity,"Please check your internet connection & restart app!");
        }
        dataFetchListener.onSuccess(response);
    }
    ...
    void onFinishFail(String errorMessage) {
        ...
        dataFetchListener.onFail(errorMessage);
    }
}
